...
type InvItem = { id: string; name: string; sku?: string; category?: string; current_stock: number; reorder_level: number; unit?: string; cost_price?: number; last_updated?: string }
type TxRecord = { id: string; item_name: string; txn_type: string; qty: number; notes?: string; created_at: string }
...
export default function PortalInventory() {
    ...
    async function load() {
      ...
      try {
        const [iRes, lRes, tRes] = await Promise.all([
          fetch(`${API}/api/v1/inventory/items?limit=200`, { headers: authHeaders() }),
          fetch(`${API}/api/v1/inventory/low-stock`, { headers: authHeaders() }),
  ...
  const filtered = search
    ? items.filter(i => i.name.toLowerCase().includes(search.toLowerCase()) || i.sku?.includes(search))
    : items

  const TXN_COLORS: Record<string, string> = {
    purchase: '#16A34A', sale: '#EF4444', adjustment: '#F59E0B', transfer: '#8B5CF6'
  }
            ...
            {(tab === 'low' ? lowStock : filtered).length === 0 ? (
              ...
              <div key={item.id} className={`flex items-center justify-between rounded-xl px-4 py-3 border
                ...
                <div className="text-right">
                  <p className={`text-sm font-bold ${item.current_stock <= item.reorder_level ? 'text-amber-400' : 'text-white'}`}>
                    {item.current_stock <= item.reorder_level && <AlertTriangle className="w-3.5 h-3.5 inline mr-1" />}
                    {item.current_stock} {item.unit || ''}
                  </p>
                  <p className="text-[10px] text-gray-500">Reorder at: {item.reorder_level}</p>
                  {item.cost_price && <p className="text-[10px] text-gray-600">Cost: ₹{item.cost_price}</p>}
                </div>
              </div>
            ))}
          </div>
            ...
            <div key={t.id} className="flex items-center justify-between bg-white/5 border border-white/10 rounded-xl px-4 py-3">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center"
                  style={{ backgroundColor: (TXN_COLORS[t.txn_type] || '#6B7280') + '20' }}>
                  <ArrowUpDown className="w-4 h-4" style={{ color: TXN_COLORS[t.txn_type] || '#6B7280' }} />
                </div>
                <div>
                  <p className="text-sm font-semibold text-white">{t.item_name}</p>
                  <p className="text-xs text-gray-500">{t.notes || t.txn_type} · {new Date(t.created_at).toLocaleDateString('en-IN')}</p>
                </div>
              </div>
              <div className="text-right">
                <p className={`text-sm font-bold ${t.txn_type === 'purchase' ? 'text-green-400' : 'text-red-400'}`}>
                  {t.txn_type === 'purchase' ? '+' : '-'}{t.qty}
                </p>
                <span className="text-[10px] font-bold capitalize px-2 py-0.5 rounded-full"
                  style={{ backgroundColor: (TXN_COLORS[t.txn_type] || '#6B7280') + '20', color: TXN_COLORS[t.txn_type] || '#6B7280' }}>
                  {t.txn_type}
                </span>
              </div>
            </div>
